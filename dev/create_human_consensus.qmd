...
WORKING_DIRECTORY="napistu_data/human_consensus2"
...
```{bash load_reactome}
source .rvars
OUTPUT_URI = $CACHE_DIR_PATH/reactome
echo "Downloading Reactome data to $OUTPUT_URI"

python -m napistu ingestion reactome $OUTPUT_URI
```
...
python -m napistu integrate reactome --permissive --overwrite $PW_INDEX $ORGANISMAL_SPECIES $OUTPUT_URI
```
...
echo "Downloading Reactome functional interactions to $REACTOME_FI_TSV"
python -m napistu ingestion reactome_fi $REACTOME_FI_TSV

echo "Integrating Reactome functional interactions from $REACTOME_FI_TSV into an SBML_dfs model at $OUTPUT_URI"
...
python -m napistu integrate bigg $CACHE_DIR_PATH/bigg/pw_index.tsv $ORGANISMAL_SPECIES $CACHE_DIR_PATH/bigg.pkl
# python -m napistu refine expand_identifiers --ontologies "ensembl_gene" --preferred_method "bioconductor" $CACHE_DIR_PATH/bigg.pkl $ORGANISMAL_SPECIES $CACHE_DIR_PATH/bigg_w_ids.pkl
```
...
```{bash string}
source .rvars
echo "Downloading results from STRING and HPA and integrating them to create a STRING sbml_dfs model at $CACHE_DIR_PATH/hpa_filtered_string.pkl for $ORGANISMAL_SPECIES species"

python -m napistu ingestion string_db $ORGANISMAL_SPECIES $CACHE_DIR_PATH/string_db
python -m napistu ingestion string_aliases $ORGANISMAL_SPECIES $CACHE_DIR_PATH/string_aliases
python -m napistu integrate string_db $CACHE_DIR_PATH/string_db $CACHE_DIR_PATH/string_aliases $ORGANISMAL_SPECIES $CACHE_DIR_PATH/string.pkl
python -m napistu ingestion proteinatlas_subcell $CACHE_DIR_PATH/hpa_subcell.tsv
...
echo "Downloading $ORGANISMAL_SPECIES IntAct results $OUTPUT_URI"
...
python -m napistu integrate omnipath -o $ORGANISMAL_SPECIES $OUTPUT_URI
```
...
python -m napistu integrate trrust --overwrite $CACHE_DIR_PATH/trrust.tsv $OUTPUT_URI
```
...
echo "Creating a dogmatic scaffold sbml_dfs model and saving results to $OUTPUT_URI"
...
python -m napistu helpers validate_sbml_dfs $CACHE_DIR_PATH/intact.pkl
python -m napistu helpers validate_sbml_dfs $CACHE_DIR_PATH/omnipath.pkl
python -m napistu helpers validate_sbml_dfs $CACHE_DIR_PATH/reactome_fi.pkl
```
...
and saving results to $output_uri"

python -m napistu consensus create --nondogmatic $reactome_input $string_input $trrust_input $dogma_input $bigg_input $intact_input $omnipath_input $reactome_fi_input $output_uri
```
