...
source .rvars
python -m napistu load reactome $CACHE_DIR_PATH/reactome
```
...
python -m napistu load trrust $CACHE_DIR_PATH/trrust.tsv
...
python -m napistu load string-db --species "Homo sapiens" $CACHE_DIR_PATH/string_db
python -m napistu load string-aliases --species "Homo sapiens" $CACHE_DIR_PATH/string_aliases
python -m napistu integrate string-db -o $CACHE_DIR_PATH/string_db $CACHE_DIR_PATH/string_aliases $CACHE_DIR_PATH/string.pkl
python -m napistu load proteinatlas-subcell $CACHE_DIR_PATH/hpa_subcell
# requires R rcpr for filtering based on shared compartments
python -m napistu refine filter_hpa_compartments $CACHE_DIR_PATH/string.pkl $CACHE_DIR_PATH/hpa_subcell $CACHE_DIR_PATH/hpa_filtered_string.pkl
```
...
python -m napistu load bigg $CACHE_DIR_PATH/bigg
...
python -m napistu refine expand_identifiers --species "Homo sapiens" --ontologies "ensembl_gene" $CACHE_DIR_PATH/bigg.pkl $CACHE_DIR_PATH/bigg_w_ids.pkl
```
...
python -m napistu consensus create --nondogmatic $reactome_input $string_input $trrust_input $bigg_input $dogma_input $output_uri
```
...
ENTITY_SET_PATH=$CACHE_DIR_PATH/external_pathways_reactome_neo4j_members.csv
REMOTE_URL="https://storage.googleapis.com/calico-cpr-public/external_pathways/external_pathways_reactome_neo4j_members.csv"

input_uri=$CACHE_DIR_PATH/consensus.pkl
output_uri=$CACHE_DIR_PATH/consensus_w_entity_sets.pkl

if [ -f "$ENTITY_SET_PATH" ]; then
  echo "File already exists at: $ENTITY_SET_PATH"
  echo "Skipping download."
else
  echo "File not found. Downloading from Google Cloud Storage..."
  wget -O "$ENTITY_SET_PATH" "$REMOTE_URL"
fi
...
if [[ -f $input_uri ]] && [[ -f $ENTITY_SET_PATH ]]; then
    ...
    python -m napistu refine add_reactome_entity_sets $input_uri $ENTITY_SET_PATH $output_uri
else
    if [[ -f $input_uri ]]; then
...
source .rvars
# globals
CROSSREF_PATH=$CACHE_DIR_PATH/external_pathways_reactome_neo4j_crossref.csv
REMOTE_URL="https://storage.googleapis.com/calico-cpr-public/external_pathways/external_pathways_reactome_neo4j_crossref.csv"

input_uri=$CACHE_DIR_PATH/consensus_w_entity_sets.pkl
output_uri=$CACHE_DIR_PATH/consensus_w_xrefs.pkl

if [ -f "$CROSSREF_PATH" ]; then
  echo "File already exists at: $CROSSREF_PATH"
  echo "Skipping download."
else
  echo "File not found. Downloading from Google Cloud Storage..."
  wget -O "$CROSSREF_PATH" "$REMOTE_URL"
fi

if [[ -f $input_uri ]] && [[ -f $CROSSREF_PATH ]]; then
    echo "Adding additional identifiers to $input_uri from the Reactome cross-ref file at $CROSSREF_PATH and saving results to $output_uri"
    python -m napistu refine add_reactome_identifiers $input_uri $CROSSREF_PATH $output_uri
else
    if [[ -f $input_uri ]]; then
        echo "Missing Model at $input_uri"
    fi
    if [[ -f $CROSSREF_PATH ]]; then
        echo "Missing Xrefs at $CROSSREF_PATH"
    fi
fi
```
...
if [[ -f $input_uri ]] ; then
    ...
    python -m napistu exporter export_precomputed_distances -s 5 $input_uri $output_uri
else
    echo "Missing $input_uri"
fi
```
...
gsutil cp $CACHE_DIR_PATH/human_consensus.tar.gz gs://shackett-napistu-public/human_consensus.tar.gz
# gsutil cp $CACHE_DIR_PATH/human_consensus_w_distances.tar.gz gs://shackett-napistu-public/human_consensus_w_distances.tar.gz
```
