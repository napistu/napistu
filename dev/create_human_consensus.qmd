...
Within this virtual env we can access the Napistu CLI using `napistu ...`. These CLI options are configured in napistu-py's `main.py`.
...
MODEL_PRIMARY="human_consensus"
MODEL_W_PRECOMPUTED_DISTANCES="human_consensus_w_distances"

WORKING_DIRECTORY="napistu_data/$MODEL_PRIMARY"
...
rm -f .rvars || true
echo -n 'export MODEL_PRIMARY=' >> .rvars
echo $MODEL_PRIMARY >> .rvars
echo -n 'export MODEL_W_PRECOMPUTED_DISTANCES=' >> .rvars
echo $MODEL_W_PRECOMPUTED_DISTANCES >> .rvars
...
# alternative network with no reaction vertices
MODEL_NO_RXNS="human_consensus_no_rxns"
RXN_FREE_EXPORT_DIR="human_consensus_no_rxns"
RXN_FREE_GRAPH_FILE="napistu_graph.pkl"
RXN_FREE_EXPORT_DIR_PATH=$WORKING_DIRECTORY/$RXN_FREE_EXPORT_DIR
RXN_FREE_SBML_DFS_OUT_PATH=$RXN_FREE_EXPORT_DIR_PATH/$SBML_DFS_FILE
RXN_FREE_GRAPH_OUT_PATH=$RXN_FREE_EXPORT_DIR_PATH/$REGULATORY_GRAPH_FILE
RXN_FREE_IDENTIFIERS_OUT_PATH=$RXN_FREE_EXPORT_DIR_PATH/$IDENTIFIERS_FILE
mkdir -p $RXN_FREE_EXPORT_DIR_PATH
echo -n 'export MODEL_NO_RXNS=' >> .rvars
echo $MODEL_NO_RXNS >> .rvars
echo -n 'export RXN_FREE_EXPORT_DIR_PATH=' >> .rvars
echo $RXN_FREE_EXPORT_DIR_PATH >> .rvars
echo -n 'export RXN_FREE_SBML_DFS_OUT_PATH=' >> .rvars
echo $RXN_FREE_SBML_DFS_OUT_PATH >> .rvars
echo -n 'export RXN_FREE_GRAPH_OUT_PATH=' >> .rvars
echo $RXN_FREE_GRAPH_OUT_PATH >> .rvars
echo -n 'export RXN_FREE_IDENTIFIERS_OUT_PATH=' >> .rvars
echo $RXN_FREE_IDENTIFIERS_OUT_PATH >> .rvars

source .rvars
echo MODEL_PRIMARY=$MODEL_PRIMARY
echo MODEL_W_PRECOMPUTED_DISTANCES=$MODEL_W_PRECOMPUTED_DISTANCES
...
echo MODEL_NO_RXNS=$MODEL_NO_RXNS
echo RXN_FREE_EXPORT_DIR_PATH=$RXN_FREE_EXPORT_DIR_PATH
echo RXN_FREE_SBML_DFS_OUT_PATH=$RXN_FREE_SBML_DFS_OUT_PATH
echo RXN_FREE_GRAPH_OUT_PATH=$RXN_FREE_GRAPH_OUT_PATH
echo RXN_FREE_IDENTIFIERS_OUT_PATH=$RXN_FREE_IDENTIFIERS_OUT_PATH
```
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    if [[ ! -d "$REACTOME_DIR" ]]; then
        ...
        napistu ingestion reactome $REACTOME_DIR
    else
        echo "Skipping Reactome download since $REACTOME_DIR exists; manually delete this to force re-download"
    fi
        
    napistu integrate reactome --permissive --overwrite $PW_INDEX_URI $ORGANISMAL_SPECIES $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi

napistu helpers summarize_sbml_dfs $OUTPUT_URI
```
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    if [[ ! -f "$REACTOME_FI_TSV" ]]; then
        ...
        napistu ingestion reactome_fi $REACTOME_FI_TSV
    else
        echo "Skipping Reactome-FI download since $REACTOME_DIR exists; manually delete this to force re-download"
    fi
        
    napistu integrate reactome_fi --overwrite $REACTOME_FI_TSV $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    if [[ ! -d $BIGG_DIR ]]; then
        ...
        napistu ingestion bigg $BIGG_DIR
    else
        echo "Skipping BiGG download since $BIGG_DIR exists; manually delete this to force re-download"
    fi
        
    napistu integrate bigg $BIGG_DIR/pw_index.tsv $ORGANISMAL_SPECIES $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    if [[ ! -f "$STRING_DB_URI" ]]; then
        ...
        napistu ingestion string_db $ORGANISMAL_SPECIES $STRING_DB_URI
    else
    ...
    if [[ ! -f "$STRING_ALIASES_URI" ]]; then
        ...
        napistu ingestion string_aliases $ORGANISMAL_SPECIES $STRING_ALIASES_URI
    else
    ...
    if [[ ! -f "$HPA_SUBCELL_URI" ]]; then
        ...
        napistu ingestion proteinatlas_subcell $HPA_SUBCELL_URI
    else
        echo "Skipping HPA download since $HPA_SUBCELL_URI exists; manually delete this to force re-download"
    fi
      
    napistu integrate string_db --overwrite $STRING_DB_URI $STRING_ALIASES_URI $ORGANISMAL_SPECIES $STRING_PKL_URI
    napistu refine filter_hpa_compartments $STRING_PKL_URI $HPA_SUBCELL_URI $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    if [[ ! -d $XML_DIR ]]; then
        ...
        napistu ingestion intact $XML_DIR $ORGANISMAL_SPECIES
    else
        echo "Skipping IntAct download since $XML_DIR exists; manually delete this to force re-download"
    fi
        
    napistu integrate intact --overwrite $XML_DIR $ORGANISMAL_SPECIES $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    napistu integrate omnipath --overwrite $ORGANISMAL_SPECIES $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    if [[ ! -f $TRRUST_TSV_URI ]]; then
        ...
        napistu ingestion trrust $TRRUST_TSV_URI
    else
        echo "Skipping TRRUST download since $TRRUST_TSV_URI exists; manually delete this to force re-download"
    fi
        
    napistu integrate trrust --overwrite $TRRUST_TSV_URI $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    napistu integrate dogmatic_scaffold --overwrite $ORGANISMAL_SPECIES $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
for model in "${!MODEL_PAIRS[@]}"; do
    ...
    if [[ ! -f "$output_uri" ]] || [[ "$OVERWRITE" == "true" ]]; then
        ...
        napistu refine merge_model_compartments "$input_uri" "$output_uri"
    else
...
# DOGMA_INPUT=$CACHE_DIR_PATH/dogma_sbml_dfs.pkl # exclude Dogma since it won't pass validation
...
for model in "${!MODEL_PATHS[@]}"; do
    ...
    napistu helpers validate_sbml_dfs "$path"
done
```
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    napistu consensus create --nondogmatic "${input_args[@]}" "$OUTPUT_URI"
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
...
napistu consensus check $OUTPUT_URI
```
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    napistu refine drop_cofactors $INPUT_URI $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi

echo "Validating final SBML_dfs from $OUTPUT_URI"
napistu helpers validate_sbml_dfs $OUTPUT_URI
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    napistu exporter export_napistu_graph -a graph_attrs_spec.yaml -g regulatory -w mixed --deduplicate_edges $INPUT_URI $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
```
...
if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    napistu exporter export_precomputed_distances -s 3 -q 0.3 -w "['weight', 'weight_upstream']" $INPUT_URI $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
```
...
if [[ ! -d "$OUTPUT_DIR" ]] || [[ "$OVERWRITE" == "true" ]]; then
    ...
    napistu exporter export_smbl_dfs_tables --nondogmatic $INPUT_URI $OUTPUT_DIR
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
```
...
```{bash}
source .rvars
echo "Relocating select flat files from $SBML_DFS_TABLES_OUT_PATH to tarr'd human_consensus directory"
...
napistu helpers validate_assets \
    "$SBML_DFS_URI" \
...
```{bash}
source .rvars
TAR_GZ_URI="$CACHE_DIR_PATH/$MODEL_PRIMARY.tar.gz"
DISTANCE_FILENAME=$(basename "$REGULATORY_DISTANCES_OUT_PATH")

echo "Taring and g-zipping the $EXPORT_DIR_PATH (excluding $DISTANCE_FILENAME) as $TAR_GZ_URI"
tar -czvf "$TAR_GZ_URI" -C "$EXPORT_DIR_PATH" --exclude="$DISTANCE_FILENAME" --exclude=".DS_Store" .
```
...
```{bash}
source .rvars
TAR_GZ_URI=$CACHE_DIR_PATH/$MODEL_W_PRECOMPUTED_DISTANCES.tar.gz
echo "Taring and g-zipping the $EXPORT_DIR_PATH as $TAR_GZ_URI"

tar -czvf $TAR_GZ_URI -C $EXPORT_DIR_PATH --exclude=".DS_Store" .
```

### Reaction-free graphs

A graph layout which doesn't include any reaction vertices

- `-a graph_attrs_spec.yaml` specifies weighting approach (same as the primary NapistuGraph)
- `-g regulatory` layout 
- `-w mixed` weighting combines numeric STRING weights and constant weights for other sources
- `-x always` drop reaction reaction vertices

```{bash export_rxn_free_napistu_graph}
source .rvars
INPUT_URI=$SBML_DFS_OUT_PATH
OUTPUT_URI=$RXN_FREE_GRAPH_OUT_PATH
OVERWRITE="true"

if [[ ! -f "$OUTPUT_URI" ]] || [[ "$OVERWRITE" == "true" ]]; then
    echo "Generating a regulatory graph with no reaction vertices from $INPUT_URI and saving results to $OUTPUT_URI"
    napistu exporter export_napistu_graph -a graph_attrs_spec.yaml -g regulatory -w mixed -x always $INPUT_URI $OUTPUT_URI
else
    echo "File exists and OVERWRITE=false, skipping ingestion/integration"
fi
```
...
```{bash}
source .rvars
TAR_GZ_URI="$CACHE_DIR_PATH/$MODEL_NO_RXNS.tar.gz"
echo "Relocating SBML_dfs from $SBML_DFS_OUT_PATH to tarr'd human_consensus directory"
cp $SBML_DFS_OUT_PATH $RXN_FREEw_SBML_DFS_OUT_PATH

echo "Relocating select flat files from $SBML_DFS_TABLES_OUT_PATH to tarr'd human_consensus directory"
cp $SBML_DFS_TABLES_OUT_PATH/species_identifiers.tsv $RXN_FREE_IDENTIFIERS_OUT_PATH

tar -czvf $TAR_GZ_URI -C $RXN_FREE_EXPORT_DIR_PATH --exclude=".DS_Store" .
```
...
if [[ "$MANUAL_TRIGGER" == "true" ]]; then
    ...
    gsutil -h "Cache-Control:no-cache" cp $CACHE_DIR_PATH/$MODEL_PRIMARY.tar.gz gs://shackett-napistu-public/$MODEL_PRIMARY.tar.gz
    gsutil -h "Cache-Control:no-cache" cp $CACHE_DIR_PATH/$MODEL_W_PRECOMPUTED_DISTANCES.tar.gz gs://shackett-napistu-public/$MODEL_W_PRECOMPUTED_DISTANCES.tar.gz
    gsutil -h "Cache-Control:no-cache" cp $CACHE_DIR_PATH/$MODEL_NO_RXNS.tar.gz gs://shackett-napistu-public/$MODEL_NO_RXNS.tar.gz
fi
```