...
This workflow is intended to create a simple pathway representation and its related outputs for testing and demonstrating Napistu's functionality.
...
napistu integrate reactome --overwrite $CACHE_DIR_PATH/reactome/pw_index_metabolism.tsv $ORGANISMAL_SPECIES $OUTPUT_URI
```
```{bash model_summary}
source .rvars_testpw
...
napistu helpers summarize_sbml_dfs $input_uri

echo "Performing post-consensus validation of the $input_uri consensus"
napistu consensus check $input_uri
```
...
if [[ -f $input_uri ]] ; then
    ...
    napistu exporter export_napistu_graph -g regulatory --deduplicate_edges $input_uri $output_uri
else
...
napistu exporter export_precomputed_distances -s 10 $input_uri $output_uri
```
...
napistu exporter export_smbl_dfs_tables $input_uri $output_uri
```
